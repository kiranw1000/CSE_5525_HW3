...
def normal_collate_fn(batch):
    ...
    encoder_ids = torch.squeeze(pad_sequence(temp, padding_value=PAD_IDX))
    encoder_mask = torch.squeeze(pad_sequence([batch[i][0]['attention_mask'].T for i in range(len(batch))], padding_value=0))
    decoder_inputs = torch.squeeze(pad_sequence([batch[i][1]['input_ids'].T for i in range(len(batch))], padding_value=PAD_IDX))
    decoder_targets = torch.squeeze(pad_sequence([batch[i][1]['input_ids'].T for i in range(len(batch))], padding_value=PAD_IDX))
